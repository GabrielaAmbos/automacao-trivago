...
| `HEADLESS=false mvn test` | Abre a janela do navegador |
| `mvn test -Dheadless=false` | Abre a janela do navegador |

### Escolher o navegador

Por padrão os testes rodam no **Chrome**. Para usar outro navegador:

| Comando | Navegador |
| ------- | --------- |
| `mvn test` | Chrome (padrão) |
| `mvn test -Dbrowser=firefox` | Firefox |
| `mvn test -Dbrowser=edge` | Edge |

Também por variável de ambiente: `BROWSER=firefox mvn test`. O navegador escolhido precisa estar **instalado**; o Selenium Manager baixa o driver correspondente automaticamente. As opções combinam: `mvn test -Dbrowser=firefox -Dheadless=false`.

Os relatórios de execução ficam em `target/surefire-reports/`. Em caso de falha, um screenshot do momento do erro é salvo em `target/screenshots/<cenário>.png` para facilitar o diagnóstico.
...
- O navegador padrão é o **Chrome**. Firefox e Edge também são suportados via `-Dbrowser` (veja [Escolher o navegador](#escolher-o-navegador)).
